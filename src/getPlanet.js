// - getPlanet is a function that creates a planet
// - children is an array of children objects
// - distance is the distance from the sun
// - img is the image of the planet
// - size is the size of the planet
...
function getPlanet({ children = [], distance = 0, img = '', size = 1 }) {
    ...
    const path = `./textures/${img}`; // path for the planets
    ...
    const rate = Math.random() * 1 - 1.0; //Calculates the rate of rotation of the planet
    orbitGroup.userData.update = (t) => { //Updates the rotation of the planet
      orbitGroup.rotation.y = t * rate; //Rotates the planet
      children.forEach((child) => {
